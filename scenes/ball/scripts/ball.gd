...
const boost_factor_base: float = 1.15
...
func color_base_on_velocity() -> void:
	var val = remap(velocity.length(), speed, max_speed, 0, 1)
...
func start_hitstop(hitstop: int) -> void:
